...
class GroundPass:
    ...
    def __init__(
        self,
        period: float = config.ORBIT_PERIOD,
        duration: float = config.PASS_DURATION,
    ...
    def info(self, t: float) -> PassInfo:
        ...
        if in_pass:
            ...
            phase = (
                ...
                and phase_t - self.duration < config.LOS_PHASE_DURATION
                and phase_t >= self.duration
                else PassPhase.PRE_PASS
            )
        ...
        to_aos = (
            0.0
            if in_pass
            else self.initial_time_to_aos - t if t < self.initial_time_to_aos else self.period - phase_t
        )
        return PassInfo(
            phase,
            in_pass,
            angle,
            phase_t / self.duration if in_pass else 0.0,
            to_aos,
            max(0.0, self.duration - phase_t) if in_pass else 0.0,
            sunlit,
            max(0, math.floor((t - self.initial_time_to_aos) / self.period) + 1),
        )
    ...
    def signal_profile(self, info: PassInfo, rng: np.random.Generator) -> float:
        ...
        signal = (
            ...
            + (config.PEAK_PASS_SIGNAL - config.MIN_PASS_SIGNAL) * math.sin(math.pi * info.progress) ** 0.8
        )
    ...
    def info(self, t: float) -> PassInfo:
        ...
        for index, (start, end) in enumerate(self.windows):
            if start <= t <= end:
                ...
                return PassInfo(
                    phase, True, angle, progress, 0.0, max(0, end - t), self._sunlit(angle), index + 1
                )
        ...
        if future:
            ...
            phase = (
                PassPhase.LOS
                if index and t - previous_end < config.LOS_PHASE_DURATION
                else PassPhase.PRE_PASS
            )
            ...
            angle = (
                self.station_angle
                + self.half_arc
                + (360 - 2 * self.half_arc) * clamp((t - previous_end) / gap, 0, 1)
            ) % 360
        ...
        phase = (
            PassPhase.LOS if self.windows and t - last_end < config.LOS_PHASE_DURATION else PassPhase.PRE_PASS
        )
