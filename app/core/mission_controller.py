"""Mission lifecycle and worker orchestration for live and replay operations."""
...
from app.models.enums import IncidentSeverity, IncidentType, MissionMode, MissionState, PacketPriority, PacketStatus, PacketType
...
class MissionController:
    ...
    def __init__(self, db: DatabaseManager, bus: EventBus, rng_seed: int | None = None):
        ...
        self._counters = {key: 0 for key in ("generated", "sent", "failed", "deferred", "dropped", "corrupted", "decisions", "incidents", "retries")}
    ...
    def mission_time(self) -> float:
        ...
        with self._lock:
            ...
            return self._replay_row_time + min(1.0, max(0.0, time.monotonic() - self._replay_row_wall) * self._replay.speed)
    ...
    def _start(self, mode: MissionMode, name: str) -> None:
        ...
        with self._lock:
            ...
            self._active = True
            self.bus.publish(EventType.MISSION_STARTED, mission_id=self._mission_id,
                             mission_code=self._mission_code, mode=mode.value, name=name)
    ...
    def start_live_mission(self) -> None:
        ...
        with self._lock:
            ...
            self._simulator = TelemetrySimulator(self.satellite, self.ground_pass, self.rng)
            for kind, priority, size in ((PacketType.HOUSEKEEPING, PacketPriority.HIGH, 4),
    ...
    def stop_mission(self, status: str = "COMPLETED") -> None:
        ...
        with self._lock:
            ...
            self._active = False
            self.bus.publish(EventType.MISSION_ENDED, mission_id=self._mission_id,
                             mission_code=self._mission_code, mode=self._mode.value, status=status)
    ...
    def request_sstv_downlink(self) -> int:
        ...
        with self._lock:
            ...
            return self._add_packet(PacketType.SSTV, PacketPriority.HIGH, round(float(self.rng.uniform(low, high)), 1)).packet_id
    def skip_to_next_pass(self) -> float:
        ...
        with self._lock:
            ...
            return advance
    def _record_incident(self, kind: IncidentType, severity: IncidentSeverity, description: str,
                         resolved: bool = False) -> None:
        incident = self.incidents.record(self._mission_id, kind, severity, description, self.mission_time(), resolved)
    ...
    def _resolve_incidents(self, kind: IncidentType) -> None:
        for incident in self.incidents.resolve_open(self._mission_id, kind):
            self.bus.publish(EventType.INCIDENT_RESOLVED, incident_id=incident.incident_id,
    ...
    def inject_incident(self, incident_type: IncidentType) -> str:
        ...
        with self._lock:
            ...
            if kind in (IncidentType.BATTERY_DRAIN, IncidentType.THERMAL_SPIKE) and self._mode is MissionMode.REPLAY:
                ...
                target = next((p for p in packets if p.status in (PacketStatus.QUEUED, PacketStatus.DEFERRED)), None)
                if target is None:
                ...
                if self._mode is MissionMode.LIVE:
                    self.satellite.reset()
                for past in (IncidentType.BATTERY_DRAIN, IncidentType.THERMAL_SPIKE, IncidentType.SIGNAL_LOSS,
                             IncidentType.COMM_FAILURE, IncidentType.FORCE_SAFE_MODE, IncidentType.LOW_POWER,
                             IncidentType.THERMAL_ALERT, IncidentType.SAFE_MODE):
                    self._resolve_incidents(past)
                severity = IncidentSeverity.INFO
                message = "Nominal conditions restored"
            self._record_incident(kind, severity, message)
            self._log("CRITICAL" if severity is IncidentSeverity.CRITICAL else "WARNING" if severity is IncidentSeverity.WARNING else "INFO", message)
            return message
    def _quarantine(self, packet: DataPacket) -> None:
        ...
        self._counters["dropped"] += 1
        self._db("upsert_packet", self._mission_id, packet)
        self._record_incident(IncidentType.CORRUPTED_PACKET, IncidentSeverity.CRITICAL,
                              f"Packet #{packet.packet_id} failed CRC and was quarantined", True)
    ...
    def _ingest_snapshot(self, snapshot: TelemetrySnapshot, info: PassInfo) -> None:
        ...
        if self._last_phase is not None and info.phase is not self._last_phase:
            self.bus.publish(EventType.GROUND_PASS_CHANGED, old=self._last_phase.value,
                             new=info.phase.value, **{"pass": info.to_dict()})
        ...
        self.bus.publish(EventType.TELEMETRY_UPDATE, snapshot=snapshot.to_dict(),
                         **{"pass": info.to_dict()}, met=format_met(snapshot.mission_time), counters=self.counters)
        ...
        while not self._mission_stop.wait(config.TELEMETRY_INTERVAL):
            try:
                with self._lock:
                    ...
                    comm_state = "LINK DOWN" if self.comms.link_down(now) else "TRANSMITTING" if active_mode else ""
                    snapshot, info = self._simulator.step(now, now + self._orbit_offset, dt,
                        active_mode, self.state_machine.state, attenuation, len(self.queue), comm_state)
            ...
            try:
                with self._lock:
                    ...
                    if snapshot is None:
                        continue
                    now = self.mission_time()
                    transition, alerts = self.health.check(snapshot, self.comms.link_down(now), self._forced_safe)
                    ...
                    if transition:
                        ...
                        if old is MissionState.COMMUNICATION_LOSS and new is not MissionState.COMMUNICATION_LOSS:
                            self._resolve_incidents(IncidentType.COMM_FAILURE)
                            self._log("INFO", "RF link recovered")
                        automatic = {MissionState.LOW_POWER: (IncidentType.LOW_POWER, IncidentSeverity.WARNING),
                                     MissionState.THERMAL_ALERT: (IncidentType.THERMAL_ALERT, IncidentSeverity.WARNING),
                        ...
                        if tx and (tx.mode.mode_type.value not in config.STATE_ALLOWED_MODES[new.value]
                                   or new is MissionState.SAFE_MODE and tx.packet.priority is not PacketPriority.CRITICAL):
                            self.comms.abort()
            ...
            try:
                with self._lock:
                    if self._mode is MissionMode.REPLAY and self._replay.paused:
                        continue
                    now = self.mission_time()
                    if self._mode is MissionMode.LIVE and now - self._last_generation >= config.PACKET_GENERATION_INTERVAL:
                    ...
                    if snapshot is None or info is None:
                        continue
                    ctx = RouterContext(snapshot.battery, snapshot.temperature, snapshot.signal,
                        self.state_machine.state, info.in_pass, info.seconds_to_los,
                        not self.comms.link_down(now), now)
                    ...
                    if self.comms.active is None and result.decision:
                        ...
                        if packet:
                            ...
                            self._db("upsert_packet", self._mission_id, packet)
                            self.bus.publish(EventType.TRANSMISSION_STARTED, packet=packet.to_record(now),
                                             mode=tx.mode.mode_type.value, estimated_time=tx.estimated_time)
                            self._log("ROUTER", f"Selected {packet.label}: score {decision.total_score:.1f}")
                    records = [p.to_record(now) for p in sorted(self.queue.snapshot(), key=lambda p: p.score, reverse=True)]
                    self.bus.publish(EventType.QUEUE_UPDATE, packets=records, hold_reason=result.hold_reason,
                        active_rules=result.active_rules, transmitting_id=self.comms.active.packet.packet_id if self.comms.active else None)
            except Exception:
        ...
        while not self._mission_stop.wait(config.TRANSMISSION_TICK):
            try:
                with self._lock:
                    ...
                    if tx is None:
                        continue
                    scale = self._replay.speed if self._mode is MissionMode.REPLAY and not self._replay.paused else 0.0 if self._mode is MissionMode.REPLAY else 1.0
                    if scale == 0:
                        continue
                    now = self.mission_time()
                    ...
                    try:
                        ...
                        if requeued:
                            ...
                            self.queue.remove(packet.packet_id)
                        self._db("upsert_packet", self._mission_id, packet)
                        self.bus.publish(EventType.TRANSMISSION_FAILED, packet=packet.to_record(now),
                            mode=tx.mode.mode_type.value, progress=tx.progress, reason=str(exc), requeued=requeued)
                    ...
                    self.bus.publish(EventType.TRANSMISSION_PROGRESS, packet_id=tx.packet.packet_id,
                        mode=tx.mode.mode_type.value, progress=tx.progress, sent_kb=tx.sent_kb,
                        total_kb=tx.total_kb, signal=signal, eta=eta, noise=noise)
                    if tx.done:
                        ...
                        self._db("upsert_packet", self._mission_id, packet)
                        self.bus.publish(EventType.TRANSMISSION_COMPLETE, packet=packet.to_record(now),
                                         mode=tx.mode.mode_type.value, duration=now - tx.started_at)
                        self._log("INFO", f"{packet.label} transmitted ({packet.size_kb:g} KB, {now - tx.started_at:.1f} s)")
            except Exception:
    ...
    def _replay_row(self, row: dict) -> None:
        with self._lock:
            ...
            signal = float(row["signal"]) * (config.SIGNAL_LOSS_ATTENUATION if now < self._signal_loss_until else 1.0)
            battery = float(row["battery"])
            voltage = config.BATTERY_VOLTAGE_EMPTY + (config.BATTERY_VOLTAGE_FULL - config.BATTERY_VOLTAGE_EMPTY) * battery / 100
            snapshot = TelemetrySnapshot(now, battery, voltage, float(row["temp"]), signal,
                float(row["power_draw"]), float(row["packet_loss"]), len(self.queue),
                self.state_machine.state.value, "LINK DOWN" if self.comms.link_down(now) else "LINK READY" if info.in_pass else "NO PASS",
    ...
    def _replay_worker(self) -> None:
        try:
            self._replay.run(self._mission_stop, self._replay_row,
