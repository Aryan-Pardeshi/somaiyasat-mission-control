...
class TelemetrySimulator:
    ...
    def step(
        ...
        orbit_time: float,
        dt: float,
        active_mode: CommunicationMode | None,
        state: MissionState,
        signal_attenuation: float = 1.0,
        queue_size: int = 0,
        comm_state: str = "",
    ) -> tuple[TelemetrySnapshot, PassInfo]:
        """Calculate one simulated frame; an injected bad frame tests recovery."""
        info = self.ground_pass.info(orbit_time)
        draw = (
            config.SAFE_MODE_POWER_DRAW
            if state is MissionState.SAFE_MODE
            else config.BASE_POWER_DRAW + (active_mode.power_draw if active_mode else 0.0)
        )
        ...
        temperature = self.satellite.thermal.update(
            draw, info.sunlit, dt, float(self.rng.normal(0, config.THERMAL_NOISE_STD))
        )
        signal = self.ground_pass.signal_profile(info, self.rng) * signal_attenuation
        loss = (
            clamp(1 + 25 * (1 - signal / 100) ** 2 + float(self.rng.normal(0, 0.6)), 0, 100)
            if info.in_pass
            else 0.0
        )
