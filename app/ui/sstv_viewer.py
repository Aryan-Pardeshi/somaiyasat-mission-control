...
class SSTVViewer(Card):
    ...
    def __init__(self, parent, source: Image.Image) -> None:
        ...
        self.screen = tk.Canvas(downlink, bg="#02050A", height=px(150), highlightthickness=1, highlightbackground=COLORS["border"])
        self.screen.pack(fill="both", expand=True)
        self.screen.bind("<Configure>", lambda _e: self._render(), add="+")
    ...
    def _render(self) -> None:
        if not self.visible or self.screen.winfo_width() < 5 or self.screen.winfo_height() < 5:
            return
        canvas_w, canvas_h = self.screen.winfo_width(), self.screen.winfo_height()
        ...
        if self._photo is None or self._photo.width() != width or self._photo.height() != height:
            ...
            if self._image_item is None:
                self._image_item = self.screen.create_image(canvas_w/2, canvas_h/2, image=self._photo)
            else:
                self.screen.itemconfigure(self._image_item, image=self._photo)
        else:
            self._photo.paste(image)
        self.screen.coords(self._image_item, canvas_w/2, canvas_h/2)
        ...
        if self.state == "receiving" and self.revealed:
            ...
            self.screen.create_line(left, y, left+width, y, fill=COLORS["cyan"], width=px(2), tags="overlay")
        if self.state in ("awaiting", "failed"):
            ...
            self.screen.create_text(canvas_w/2, canvas_h/2, text=label, fill=COLORS["muted"] if self.state == "awaiting" else COLORS["red"], font=FONTS["h2"], tags="overlay")
