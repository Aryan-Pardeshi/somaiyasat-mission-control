...
class DatabaseManager:
        ...
        try:
            with self._lock, self._conn:
                count = self._conn.execute(
                    "SELECT COUNT(*) FROM missions WHERE mission_code LIKE ?",
                    (f"{config.MISSION_CODE_PREFIX}-{today:%Y%m%d}-%",),
                ).fetchone()[0]
                code = mission_code(today, count + 1)
                cur = self._conn.execute(
                    "INSERT INTO missions(mission_code,mission_name,mode,started_at,status,notes) VALUES(?,?,?,?,?,?)",
    ...
    def end_mission(self, mission_id: int, status: str) -> None:
        ...
        self._write(
            "UPDATE missions SET ended_at=?,status=? WHERE mission_id=?", (now_iso(), status, mission_id)
        )

    def insert_telemetry(self, mission_id, snapshot) -> None:
        """Persist the sensor values used by analytics."""
        self._write(
            "INSERT INTO telemetry(mission_id,timestamp,mission_time,battery,temperature,signal,power_draw,packet_loss,system_state) VALUES(?,?,?,?,?,?,?,?,?)",
            # Values are rounded for storage: sensors are not more precise than this.
            (
                mission_id,
                snapshot.timestamp,
                round(snapshot.mission_time, 2),
                round(snapshot.battery, 2),
                round(snapshot.temperature, 2),
                round(snapshot.signal, 2),
                round(snapshot.power_draw, 3),
                round(snapshot.packet_loss, 2),
    ...
    def upsert_packet(self, mission_id, packet) -> None:
        ...
        self._write(
            """INSERT INTO packets(packet_id,mission_id,packet_type,priority,size_kb,status,created_at,
            ...
            (
                ...
                packet.retry_count,
                int(packet.corrupted),
                packet.mode_used,
                round(packet.score, 2),
            ),
        )
    ...
    def insert_decision(self, mission_id, decision) -> int:
        ...
        return self._write(
            """INSERT INTO decisions(mission_id,packet_id,timestamp,mission_time,selected_mode,
            ...
            (
                ...
                decision.packet_id,
                decision.timestamp,
                round(decision.mission_time, 2),
                decision.selected_mode,
                round(decision.total_score, 2),
                round(decision.priority_score, 2),
                round(decision.link_score, 2),
                round(decision.urgency_score, 2),
                round(decision.energy_score, 2),
                round(decision.waiting_score, 2),
                round(decision.battery, 2),
                round(decision.temperature, 2),
                round(decision.signal, 2),
    ...
    def insert_incident(self, mission_id, incident) -> int:
        ...
        return self._write(
            "INSERT INTO incidents(mission_id,timestamp,mission_time,incident_type,severity,description,resolved) VALUES(?,?,?,?,?,?,?)",
            (
                mission_id,
                incident.timestamp,
                incident.mission_time,
    ...
    def insert_state_transition(self, mission_id, old, new, reason, mission_time) -> None:
        ...
        self._write(
            "INSERT INTO state_transitions(mission_id,timestamp,mission_time,from_state,to_state,reason) VALUES(?,?,?,?,?,?)",
    ...
    def get_missions(self, search: str = "", mode: str | None = None) -> list[dict]:
        ...
        rows = self._read(
            """SELECT m.*, (SELECT COUNT(*) FROM packets p WHERE p.mission_id=m.mission_id) packets,
        ...
        try:
            with self._lock:
                return pd.read_sql_query(
                    f"SELECT * FROM {table} WHERE mission_id=?", self._conn, params=(mission_id,)
                )
