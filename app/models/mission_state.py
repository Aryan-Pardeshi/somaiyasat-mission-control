...
class StateMachine:
    ...
    def evaluate(
        ...
        keep_safe = self.state is MissionState.SAFE_MODE and (
            battery < config.BATTERY_CRITICAL_THRESHOLD + config.BATTERY_RECOVERY_MARGIN
            or temperature > config.TEMP_WARNING_THRESHOLD
        )
        if (
            forced_safe
            or battery < config.BATTERY_CRITICAL_THRESHOLD
            or temperature > config.TEMP_CRITICAL_THRESHOLD
            or keep_safe
        ):
        ...
        if (
            temperature > config.TEMP_WARNING_THRESHOLD
            or self.state is MissionState.THERMAL_ALERT
            and temperature > config.TEMP_WARNING_THRESHOLD - config.TEMP_RECOVERY_MARGIN
        ):
            return MissionState.THERMAL_ALERT
        if (
            battery < config.BATTERY_LOW_THRESHOLD
            or self.state is MissionState.LOW_POWER
            and battery < config.BATTERY_LOW_THRESHOLD + config.BATTERY_RECOVERY_MARGIN
        ):
    ...
    def reason_for(
        ...
        if state is MissionState.SAFE_MODE:
            return f"Safe mode: battery {battery:.1f} %, temperature {temperature:.1f} °C" + (
                "; operator forced" if forced_safe else ""
            )
    ...
    def update(
        ...
        comm_failure: bool = False,
        forced_safe: bool = False,
    ) -> tuple[MissionState, MissionState, str] | None:
