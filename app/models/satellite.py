"""Simple battery and thermal models with hidden mutable state."""
...
class PowerSystem:
    ...
    def voltage(self) -> float:
        ...
        return config.BATTERY_VOLTAGE_EMPTY + (config.BATTERY_VOLTAGE_FULL - config.BATTERY_VOLTAGE_EMPTY) * self.__battery / 100
    @property
    ...
    def consume_power(self, watts: float, seconds: float) -> None:
        ...
        self.__battery = clamp(self.__battery - max(0, watts) * max(0, seconds) * config.BATTERY_PCT_PER_JOULE, 0, 100)
...
class ThermalSystem:
    ...
    def update(self, power_draw: float, sunlit: bool, dt: float, noise: float = 0.0) -> float:
        ...
        target = config.THERMAL_BASELINE + config.THERMAL_POWER_COEFF * power_draw + (config.THERMAL_SUN_OFFSET if sunlit else config.THERMAL_ECLIPSE_OFFSET) + self._external_heat
        self.__temperature += (target - self.__temperature) * min(1, config.THERMAL_RESPONSE * max(0, dt)) + noise
