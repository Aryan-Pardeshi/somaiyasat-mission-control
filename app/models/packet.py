...
class DataPacket:
    ...
    def __init__(
        ...
        try:
            ...
            valid_size = False
        if (
            not isinstance(priority, PacketPriority)
            or not isinstance(packet_id, int)
            or packet_id < 0
            or not valid_size
        ):
            raise InvalidPacketError("Packet ID, priority, or size is invalid")
        self.packet_id, self.priority, self.size_kb, self.created_at = (
            packet_id,
            priority,
            float(size_kb),
            float(created_at),
        ...
        self._payload = (
            f"{packet_id}|{self.packet_type.value}|{priority.value}|{size_kb}|{created_at}".encode()
        )
    ...
    def to_record(self, now: float | None = None) -> dict:
        ...
        return dict(
            ...
            type_label=self.packet_type.label,
            priority=self.priority.value,
            size_kb=self.size_kb,
            status=self.status.value,
            retry_count=self.retry_count,
            corrupted=bool(self.corrupted),
            score=self.score,
            hold_reason=self.hold_reason,
            age=self.age(now) if now is not None else 0.0,
            required_signal=config.MODE_MIN_SIGNAL[self.preferred_mode.value],
            mode=self.mode_used or self.preferred_mode.value,
...
PACKET_CLASSES: dict[PacketType, type[DataPacket]] = {
    PacketType.TTC: TTCPacket,
    PacketType.HOUSEKEEPING: HousekeepingPacket,
    PacketType.SSTV: SSTVPacket,
    PacketType.M17: M17Packet,
    PacketType.CODEC2: Codec2Packet,
}
...
def create_packet(
    ...
    try:
        kind = (
            packet_type if isinstance(packet_type, PacketType) else PacketType(normalize_label(packet_type))
        )
        level = (
            priority if isinstance(priority, PacketPriority) else PacketPriority(normalize_label(priority))
        )
